...
// be deleted.
//
// At present, RMC correctly reports the following
//
//    ** 5 of 26 failed (4 iterations)
//    VERIFICATION FAILED
//
// The failures are in functions t1, ... t5 and are marked with a comment
// of the form "// SHOULD ..."
...
fn t2() {
    ...
    let a: u32 = AbstractValue::abstract_value();
    let b: u32 = AbstractValue::abstract_value();
    let r = a * b; // SHOULD be able to overflow
    assume(4 <= a && a <= 7);
    assume(5 <= b && b <= 8);
    assert!(20 <= r && r <= 56);
}
