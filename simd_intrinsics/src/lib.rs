...
mod vector {
            ...
            Self::Vec: Copy,
    {
        ...
        /// Construct a vector from individual elements
        fn new(x0: Self, x1: Self, x2: Self, x3: Self, x4: Self, x5: Self, x6: Self, x7: Self,
               x8: Self, x9: Self, x10: Self, x11: Self, x12: Self, x13: Self, x14: Self, x15: Self,
               x16: Self, x17: Self, x18: Self, x19: Self, x20: Self, x21: Self, x22: Self, x23: Self,
               x24: Self, x25: Self, x26: Self, x27: Self, x28: Self, x29: Self, x30: Self, x31: Self) -> Self::Vec;
    ...
    macro_rules! vector32 {
        ($ety: ty, $pty: ident, $mty: ty) => {
            ...
            impl Vector32 for $ety {
                ...
                fn get14(x: Self::Vec) -> Self { x.14 }
                fn get15(x: Self::Vec) -> Self { x.15 }
                fn get16(x: Self::Vec) -> Self { x.16 }
                fn get17(x: Self::Vec) -> Self { x.17 }
                fn get18(x: Self::Vec) -> Self { x.18 }
                fn get19(x: Self::Vec) -> Self { x.19 }
                fn get20(x: Self::Vec) -> Self { x.20 }
                fn get21(x: Self::Vec) -> Self { x.21 }
                fn get22(x: Self::Vec) -> Self { x.22 }
                fn get23(x: Self::Vec) -> Self { x.23 }
                fn get24(x: Self::Vec) -> Self { x.24 }
                fn get25(x: Self::Vec) -> Self { x.25 }
                fn get26(x: Self::Vec) -> Self { x.26 }
                fn get27(x: Self::Vec) -> Self { x.27 }
                fn get28(x: Self::Vec) -> Self { x.28 }
                fn get29(x: Self::Vec) -> Self { x.29 }
                fn get30(x: Self::Vec) -> Self { x.30 }
                fn get31(x: Self::Vec) -> Self { x.31 }
...
mod scalar {
    ...
    pub fn srl_immed_u32_u8(x: u32, imm8: u8) -> u32 {
        if imm8 > 31 { 0 } else { x >> imm8 }
    }

    /// Logical shift right by 8-bit immediate (0 if shift distance too large)
    pub fn srl_immed_u64_u8(x: u64, imm8: u8) -> u64 {
        if imm8 > 63 { 0 } else { x >> imm8 }
    }
